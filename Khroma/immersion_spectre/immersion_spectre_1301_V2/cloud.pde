...
void draw_cloud(){
      ...
      for(int h = 0 ; h<  detections.size() ; h++){
        for(int i = 0 ; i < n_particules; i++){
          ...
          if(h == 0){            
            ...
            //if(raie_H2O_position[i]
            targetX[0][i] = map(raie_H2O[raie_H2O_position[i]],0,18,0,width)+random(-mol_noise,mol_noise);
            targetY[0][i] = map(i,0,100,0,height);
          }
          //CO2
          if(h == 1){            
            int choix_raie = int(random(0,3));
            targetX[1][i] = map(raie_CO2[raie_H2O_position[i]],0,18,0,width)+random(-mol_noise,mol_noise);
            targetY[1][i] = map(i,0,100,0,height);
          }
        }
      }   
    }
  ...
  if(info_on){
    ...
    srcLive.beginDraw();
    srcLive.background(0);
    srcLive.textSize(15);
    for(int h = 0 ; h<  detections.size() ; h++){
      //fill(mol_couleur[h]);
      srcLive.fill(255);
      for(int i = 0 ; i < n_particules; i++){
        srcLive.text(detections.getString(h),x[h][i],y[h][i]);
      }
    }
    srcLive.endDraw();
}
