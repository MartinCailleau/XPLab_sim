...
void drawCore(float radius) {
  fill(0, 0, 100); // Couleur blanche pour le noyau
...
void drawCrown(float innerR, float outerR, int totalSegments, ArrayList segmentColors, float rotationAngle) {
  ...
  for (int i = 0; i < totalSegments; i++) {
    ...
    fill((int)segmentColors.get(i % segmentColors.size()));
    noStroke(); // Pas de contour pour les segments remplis

    arc(0, 0, outerR * 2, outerR * 2, startAngle, endAngle, PIE);
  }
